...
(defun day6-part-2 (input-elements)
  ...
  (let* ((tuples           (parse-tuples input-elements))
    ...
    (iter
      ...
      (with g               = graph)
      (with candidate-nodes = (list (cons "SAN" nil)))
      (incf count)
      (when (> count *bailout-count*)
        (finish))
      (while candidate-nodes)
      (initially (setq var (car candidate-nodes)))
      (for var next (car candidate-nodes))
    ...
    (iter
      ...
      (with visited         = (make-hash-table :test #'equal))
      (with g               = graph)
      (with candidate-nodes = (list (cons "YOU" nil)))
      (incf count)
      (when (> count *bailout-count*)
