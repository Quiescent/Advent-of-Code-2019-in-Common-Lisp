...
(defun interpret (program input ptr)
  (iter
    ...
    (defcpu ((1 (progn (setf (gethash d3 program 0) (+ p1 p2))
                       (return (values (+ ptr 4) nil nil nil))))
             (2 (progn (setf (gethash d3 program 0) (* p1 p2))
                       (return (values (+ ptr 4) nil nil nil))))
             (3 (if (null current-input)
                    (return (values ptr nil nil nil))
                    (progn (format t "Reading (~a)...~%" input)
                           (setf (gethash d1 program) input)
                           (return (values (+ ptr 2) nil nil t)))))
             (4 (return (values (+ ptr 2) p1 nil nil)))
             (5 (if (not (eq 0 p1))
                    (return (values p2 nil nil nil))
                    (return (values (+ ptr 3) nil nil nil))))
             (6 (if (eq 0 p1)
                    (return (values p2 nil nil nil))
                    (return (values (+ ptr 3) nil nil nil))))
             (7 (progn (setf (gethash d3 program) (if (< p1 p2) 1 0))
                       (return (values (+ ptr 4) nil nil nil))))
             (8 (progn (setf (gethash d3 program) (if (eql p1 p2) 1 0))
                       (return (values (+ ptr 4) nil nil nil))))
             (9 (progn (setf *relative-base* (+ *relative-base* p1))
                       (return (values (+ ptr 2) nil nil nil))))))
    (finally (return nil))))
