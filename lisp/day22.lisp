...
(defun day22-part-2 (input-elements)
  ...
  (let* ((number-of-cards 119315717514047)
    ...
    (labels
        ((cut (number idx)
         ...
         (inc (number idx)
           (let ((idx-2 idx))
                       ;; (format t "Incing: number-of-cards: ~a, num: ~a, mod: ~a~%"
                       ;;         number-of-cards idx number)
                       (declare (type fixnum idx-2))
                       (declare (type fixnum number))
                       (solve-modulo number-of-cards idx-2 number)))
         (rev (idx)
           (let ((idx-2 idx))
             ;; (format t "Reving: rev: ~a, idx: ~a~%" idx number-of-cards)
             (declare (type fixnum idx-2))
             (- (1- number-of-cards) idx-2)))
         (solve (x i)
      ...
      (solve 2020 (1- (* 100 instructions-length))
